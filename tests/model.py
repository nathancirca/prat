...
class AE(torch.nn.Module):
    ...
    def forward(self,x ,batch_size):
        ...
        """
        x=torch.reshape(x, [batch_size, 2, 256, 256])
        skips = [x]
        ...
        return x[:,0,...].reshape((2,1,256,256)) - n
    ...
    def loss_function(self,output,target,batch_size):
      ...
      """
      loss = torch.mean(torch.square(torch.abs(output-target)))
      return loss
    ...
    def training_step(self, batch,batch_number):
      ...
      x = batch
      print(x.shape)
      y1 = self.generate_speckle(x,1)
      y2 = self.generate_speckle(x,20)
      y = torch.cat((y1,y2),1)
      print(y.shape)
    ...
    def validation_step(self, batch,image_num,epoch_num,eval_files,eval_set,sample_dir):
      ...
      print("here",outputimage.shape, noisyimage.shape)
      save_sar_images(np.transpose(outputimage,(1,2,0)), np.transpose(noisyimage,(1,2,0)), imagename,sample_dir)
