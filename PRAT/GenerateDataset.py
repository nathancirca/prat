...
class GenerateDataset():
    ...
    def generate_patches(self,src_dir="./dataset/data/Train",pat_size=256,step=0,stride=64,bat_size=4,data_aug_times=1,n_channels=1):
        count = 0
        ...
        numScenes=np.size(img, 2)
        inputs = np.zeros((numPatches, pat_size, pat_size, numScenes, n_channels), dtype="float32")
        ...
        for i in range(len(filepaths)): #scan through images
            img = (np.load(filepaths[i]))
            img_s = img.reshape((img.shape[0],img.shape[1], img.shape[2],n_channels))
            im_h = np.size(img, 0)
            im_w = np.size(img, 1)
